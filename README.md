...
### Method 1: Enhanced Parser (Attempts Automated Scraping)
1. **Run the main parser:**
```bash
python h1b_job_parser.py
```
*Note: May encounter 403 blocking errors from job sites*

### Method 2: Practical Manual Approach (Recommended)
1. **Run the practical parser:**
```bash
python simple_working_h1b_parser.py
```
*Generates real job leads and manual search guidance*

2. **View results:**
- Check generated CSV/JSON files in the current directory
- Review the console output for summary statistics
- Follow the manual search guide for best results

3. **Expected output files:**
- `simple_h1b_jobs_YYYYMMDD_HHMMSS.csv` - Job leads and company info
- `simple_h1b_jobs_YYYYMMDD_HHMMSS.json` - Same data in JSON format

## 🛡️ Anti-blocking Solutions

If you encounter **403 Forbidden** errors with `h1b_job_parser.py`, try these solutions:

### Solution 1: Use Practical Parser (Recommended)  
```bash
# This generates real job leads without scraping
python simple_working_h1b_parser.py
```
...
### Solution 3: Manual Search (Most Effective)
- Use LinkedIn job alerts
- Search company career pages directly  
- Check H1B database sites (MyVisaJobs.com, H1BGrader.com)
- Follow the manual search guide generated by the practical parser

## 🔄 Practical Parser Features

The `simple_working_h1b_parser.py` script includes:

- **🏢 Real Company URLs**: Direct links to career pages of H1B sponsors
- **📊 H1B Database Integration**: Companies confirmed to sponsor visas
- **📋 Manual Search Guide**: Step-by-step job hunting strategy  
- **🎯 Practical Approach**: Focuses on what actually works
- **✅ No Blocking Issues**: Doesn't rely on scraping job boards

### Practical Parser Usage
```bash
# Run the practical approach
python simple_working_h1b_parser.py

# Expected benefits:
# - Real company career page URLs
# - Confirmed H1B sponsor companies
# - Manual verification guidance
# - Higher success rate than automated scraping
```
...
### Basic Usage
```bash
# Enhanced parser (may get blocked)
python h1b_job_parser.py

# Practical parser (recommended)  
python simple_working_h1b_parser.py
```
...
parser = H1BJobParser()
jobs = parser.scrape_indeed("DevOps Engineer", max_pages=2)
h1b_jobs = parser.filter_h1b_jobs(jobs)
parser.save_results(h1b_jobs)
```

### Practical Parser Usage
```python  
from simple_working_h1b_parser import SimpleH1BJobParser

parser = SimpleH1BJobParser()

# Search major company career pages
company_jobs = parser.search_company_jobs("Google", parser.h1b_companies["Google"])

# Generate H1B database leads
db_jobs = parser.search_h1b_database_companies()

# Get manual search guide
guide = parser.get_manual_search_guide()

# Combine all leads
all_jobs = company_jobs + db_jobs + guide
...
### v2.0.0 (Current - Practical Approach)
- ✅ Added practical manual search parser (`simple_working_h1b_parser.py`)
- ✅ Real company career page URLs and H1B sponsor database
- ✅ Comprehensive manual search guide with proven strategies
- ✅ Removed buggy alternative parsers and consolidated codebase
- ✅ Focus on what actually works: direct applications and networking

### v1.3.0 (Anti-blocking Update)
